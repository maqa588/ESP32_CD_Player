...
void mdns_airplay_init(void) {
  ...
  if (err != ESP_OK) {
    ...
    ESP_LOGI(TAG, "Successfully added mDNS _airplay._tcp service for [%s] on port 7000", device_name);
  }
#endif
  ...
  if (err_raop != ESP_OK) {
    ...
    ESP_LOGI(TAG, "Successfully added mDNS _raop._tcp service for [%s] on port 7000", service_name);
  }
}
