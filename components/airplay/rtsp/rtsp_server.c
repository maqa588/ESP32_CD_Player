...
#define CLIENT_STACK_SIZE 12288
...
static void server_task(void *pvParameters) {
  ...
  while (server_running) {
    ...
    BaseType_t task_ret =
        xTaskCreatePinnedToCoreWithCaps(client_task, "rtsp_client", CLIENT_STACK_SIZE,
                    (void *)(intptr_t)new_slot, 5, &clients[new_slot].task, 0,
                    MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (task_ret != pdPASS || clients[new_slot].task == NULL) {
      task_ret = xTaskCreate(client_task, "rtsp_client", CLIENT_STACK_SIZE,
...
esp_err_t rtsp_server_start(void) {
  ...
  BaseType_t task_ret =
      xTaskCreatePinnedToCoreWithCaps(server_task, "rtsp_server", SERVER_STACK_SIZE, NULL, 5,
                                      &server_task_handle, 0, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (task_ret != pdPASS || server_task_handle == NULL) {
    ESP_LOGW(TAG, "Failed to create rtsp_server task in SPIRAM, retrying with internal memory...");
    task_ret = xTaskCreatePinnedToCore(server_task, "rtsp_server", 4096, NULL, 5,
                                       &server_task_handle, 0);
  }
  if (task_ret != pdPASS || server_task_handle == NULL) {
    ESP_LOGE(TAG, "Failed to create rtsp_server task");
    return ESP_FAIL;
  }
  ESP_LOGI(TAG, "rtsp_server task created successfully on Core 0");
  return ESP_OK;
}
