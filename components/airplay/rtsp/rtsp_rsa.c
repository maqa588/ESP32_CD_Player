...
// Parsed RSA context (initialized once)
...
static int ensure_pk_initialized(void) {
  ...
  int ret = mbedtls_pk_parse_key(&s_pk_ctx,
...
int rsa_apple_challenge_response(const char *challenge_b64, uint32_t ip_addr,
  ...
  size_t rsa_len = (mbedtls_pk_get_bitlen(&s_pk_ctx) + 7) / 8;
  if (rsa_len == 0) {
    rsa_len = 256;
  }
  ...
  size_t sig_len = 0;
  int ret = mbedtls_pk_sign(&s_pk_ctx, MBEDTLS_MD_NONE, data, sizeof(data),
                            rsa_out, rsa_len, &sig_len);
  if (ret != 0) {
  ...
  ret = b64_encode(rsa_out, sig_len, out_b64, out_b64_size);
  free(rsa_out);
...
int rsa_decrypt_aes_key(const char *encrypted_b64, uint8_t *out_key,
  ...
  // RSA OAEP-SHA1 decrypt via PSA Crypto API
  mbedtls_svc_key_id_t key_id = s_pk_ctx.priv_id;
  psa_status_t status = psa_asymmetric_decrypt(
      key_id, PSA_ALG_RSA_OAEP(PSA_ALG_SHA_1),
      encrypted, encrypted_len,
      NULL, 0,
      out_key, out_key_size, out_key_len);

  if (status != PSA_SUCCESS) {
    ESP_LOGE(TAG, "RSA AES key decrypt failed: %ld", (long)status);
    return -1;
  }

  ESP_LOGI(TAG, "Decrypted AES key: %zu bytes", *out_key_len);
  return 0;
}
