...
#define AUDIO_BUFFERED_STACK_SIZE  8192
...
static void buffered_audio_task(void *pvParameters) {
  ...
  while (stream->running) {
    ...
    int rcvbuf = 32768; // 32KB socket buffer for smooth TCP streaming over 2.4GHz Wi-Fi
...
static esp_err_t buffered_start(audio_stream_t *stream, uint16_t port) {
  ...
  state->buffered_task_handle = NULL;
  BaseType_t task_ret =
      task_create_pinned_spiram(buffered_audio_task, "buff_audio", AUDIO_BUFFERED_STACK_SIZE,
                                stream, 5, &state->buffered_task_handle, 0, NULL);
