...
#define MIN_STARTUP_FRAMES  50 // ~1.16s pre-buffering for stable 2.4GHz streaming
...
#else
#define TIMING_THRESHOLD_US 150000 // 150ms early/late threshold (buffered) for 2.4GHz Wi-Fi jitter
#endif

#ifdef CONFIG_AIRPLAY_RT_TIMING_THRESHOLD_MS
#define RT_TIMING_THRESHOLD_US (CONFIG_AIRPLAY_RT_TIMING_THRESHOLD_MS * 1000)
#else
#define RT_TIMING_THRESHOLD_US 200000 // 200ms early/late threshold (realtime)
#endif
  ...
  for (int attempt = 0; attempt < MAX_DRAIN_ATTEMPTS; attempt++) {
    ...
    if (timing->anchor_valid && format->sample_rate > 0) {
      ...
      if (compute_early_us(timing, format, hdr->rtp_timestamp, sync_mode,
        ...
        int64_t strict_us = (frame_period_us * 2 > 60000) ? (frame_period_us * 2) : 60000;
        int64_t release_us = (from_pending || gap)
                                 ? strict_us
                                 : timing_threshold_us;

        int64_t late_limit_us = -timing_threshold_us;
        ...
        if (early_us > release_us) {
          ...
          // Network underrun protection: if consecutive drops exceed 15 frames (~350ms),
          // a network stall occurred. Resync anchor to current playhead instead of discarding
          // seconds of audio (e.g. dropped=451)!
          if (timing->late_drop_count > 15) {
            ESP_LOGW(TAG, "Network underrun recovery: resyncing anchor after %u late frames", timing->late_drop_count);
            timing->anchor_valid = false;
            timing->late_drop_count = 0;
            timing->late_drop_active = false;
            if (from_pending) {
