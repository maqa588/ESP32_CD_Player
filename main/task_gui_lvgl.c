...
void task_lvgl(void *args)
{
    ...
    ESP_LOGI("task_lvgl", "gui_airplay_init.");
    gui_airplay_init();
    ...
    {
        // 模式切换视图联动
        if (app_mode_has_changed())
        {
            if (app_mode_get() == APP_MODE_CD)
            {
                gui_airplay_set_visible(false);
                gui_cdPlayer_set_visible(true);
            }
            else
            {
                gui_cdPlayer_set_visible(false);
                gui_airplay_set_visible(true);
            }
        ...
        if (app_mode_get() == APP_MODE_AIRPLAY)
        {
            ...
            static int32_t maxL_ap = 0;
            static int32_t maxR_ap = 0;
            ChannelValue_t oscilloscopePoint;
            while (xQueueReceive(queue_oscilloscope, &oscilloscopePoint, 0) == pdTRUE)
            {
                if (oscilloscopePoint.l > maxL_ap)
                    maxL_ap = oscilloscopePoint.l;
                if (oscilloscopePoint.r > maxR_ap)
                    maxR_ap = oscilloscopePoint.r;

                if ((++count_ap) == 50)
                {
                    count_ap = 0;
                    double dbL = (maxL_ap > 0) ? 20 * log10(maxL_ap) : 0;
                    double dbR = (maxR_ap > 0) ? 20 * log10(maxR_ap) : 0;
                    maxL_ap = 0;
                    maxR_ap = 0;
                    gui_airplay_set_meter((int)dbL, (int)dbR);
                    break;
                }
            }
        }
