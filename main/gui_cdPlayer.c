...
#define AREA_OSCILLOSCOPE_HEIGHT 28
...
void gui_player_init()
{
    ...
    lv_obj_set_pos(area_bottomBar, 0, LCD_H - size_barHeight);
     ...
     * Background - Player
     */
    int size_playerWidth = LCD_W;
    int size_playerHeight = LCD_H - 2 * AREA_STATUS_BAR_HEIGHT - AREA_OSCILLOSCOPE_HEIGHT;
     ...
     */
    int size_oscilloscopeWidth = LCD_W / 2;
    ...
    lv_obj_set_style_text_font(lb_albumTitle, &lv_font_montserrat_12, 0);
    lv_obj_set_width(lb_albumTitle, 110);
    lv_obj_set_height(lb_albumTitle, 14);
    ...
    lv_obj_set_style_text_font(lb_trackTitle, &lv_font_montserrat_16, 0);
    lv_obj_set_width(lb_trackTitle, LCD_W - 6);
    lv_obj_set_height(lb_trackTitle, 18);
    ...
    lv_obj_set_style_text_font(lb_trackPerformer, &lv_font_montserrat_12, 0);
    lv_obj_set_width(lb_trackPerformer, LCD_W - 6);
    lv_obj_set_height(lb_trackPerformer, 14);
    ...
    gui_setTrackTitle("Track Title", "Performer");
    ...
    lv_style_set_pad_all(&style_preEmphasis, 1);
    lv_style_set_pad_left(&style_preEmphasis, 2);
    lv_style_set_pad_right(&style_preEmphasis, 2);
    lv_style_set_text_color(&style_preEmphasis, lv_color_white());
    lv_style_set_text_font(&style_preEmphasis, &lv_font_montserrat_10);
    ...
    lv_obj_set_style_text_font(lb_time, &lv_font_montserrat_10, LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(lb_duration, &lv_font_montserrat_10, LV_PART_MAIN);
    ...
    lv_obj_set_size(bar_playProgress, LCD_W - 6, 4);
    lv_obj_align_to(bar_playProgress, area_player, LV_ALIGN_BOTTOM_MID, 0, -2);
     ...
     * oscilloscope
     */
    size_oscilloscopeWidth = LCD_W / 2 - size_meterWidth - 1;
...
void gui_setDriveModel(const char *str)
{
    ...
    lv_obj_align_to(lb_driveModel, area_topBar, LV_ALIGN_LEFT_MID, 3, 0);
}
...
void gui_setDriveState(const char *str)
{
    ...
    lv_obj_align_to(lb_driveState, area_topBar, LV_ALIGN_RIGHT_MID, -3, 0);
}
...
void gui_setAlbumTitle(const char *str)
{
    ...
    lv_obj_align_to(lb_albumTitle, area_player, LV_ALIGN_TOP_LEFT, 3, 2);
}
...
void gui_setTrackTitle(const char *title, const char *performer)
{
    ...
    if ((title != NULL) && (strcmp(oldTitle, title) != 0))
    {
        ...
        if (noPerformer)
        {
            lv_obj_set_height(lb_trackTitle, 36);
            lv_obj_align_to(lb_trackTitle, area_player, LV_ALIGN_TOP_MID, 0, 16);
        }
        else
        {
            lv_obj_set_height(lb_trackTitle, 18);
            lv_obj_align_to(lb_trackTitle, area_player, LV_ALIGN_TOP_MID, 0, 15);
        }
    }
    ...
    if ((performer != NULL) && (strcmp(oldPerformer, performer) != 0))
    {
        ...
        lv_obj_align_to(lb_trackPerformer, lb_trackTitle, LV_ALIGN_OUT_BOTTOM_MID, 0, 1);
    }
}
...
void gui_setEmphasis(bool en)
{
    ...
    lv_obj_align_to(lb_preEmphasized, area_player, LV_ALIGN_TOP_RIGHT, -3, 2);
}
...
void gui_setTime(hmsf_t current, hmsf_t total)
{
    ...
    if (memcmp(&oldCurrent, &current, sizeof(hmsf_t)) != 0)
    {
        ...
        lv_obj_align_to(lb_time, area_player, LV_ALIGN_BOTTOM_LEFT, 3, -8);
    }
    ...
    if (memcmp(&oldTotal, &total, sizeof(hmsf_t)) != 0)
    {
        ...
        lv_obj_align_to(lb_duration, area_player, LV_ALIGN_BOTTOM_RIGHT, -3, -8);
    }
}
...
void gui_setPlayState(const char *str)
{
    ...
    lv_obj_align_to(lb_playState, area_bottomBar, LV_ALIGN_LEFT_MID, 4, 0);
}
...
void gui_setVolume(int vol)
{
    ...
    lv_obj_align_to(lb_volume, area_bottomBar, LV_ALIGN_RIGHT_MID, -4, 0);
}
...
void gui_cdPlayer_set_visible(bool visible)
{
    if (visible)
    {
        if (area_topBar) lv_obj_clear_flag(area_topBar, LV_OBJ_FLAG_HIDDEN);
        if (area_bottomBar) lv_obj_clear_flag(area_bottomBar, LV_OBJ_FLAG_HIDDEN);
        if (area_player) lv_obj_clear_flag(area_player, LV_OBJ_FLAG_HIDDEN);
        if (area_left) lv_obj_clear_flag(area_left, LV_OBJ_FLAG_HIDDEN);
        if (area_right) lv_obj_clear_flag(area_right, LV_OBJ_FLAG_HIDDEN);
    }
    else
    {
        if (area_topBar) lv_obj_add_flag(area_topBar, LV_OBJ_FLAG_HIDDEN);
        if (area_bottomBar) lv_obj_add_flag(area_bottomBar, LV_OBJ_FLAG_HIDDEN);
        if (area_player) lv_obj_add_flag(area_player, LV_OBJ_FLAG_HIDDEN);
        if (area_left) lv_obj_add_flag(area_left, LV_OBJ_FLAG_HIDDEN);
        if (area_right) lv_obj_add_flag(area_right, LV_OBJ_FLAG_HIDDEN);
    }
}
